...
checkNext pos sud = checkPossible (nextPos pos) sud && checkNext (nextPos pos) sud

checkPossible :: Pos -> Sudoku-> Bool
checkPossible pos sud = (length (possibleCells pos sud)) > 0

selectCell' :: Pos -> Sudoku -> [Int] -> Cell
selectCell' pos sud [] = Nothing
selectCell' pos sud list 
    | checkNext pos (update sud pos cell) = cell
    | otherwise = selectCell' pos sud (tail list)
    where cell = Just $ head list

solveHelp :: (Sudoku, Pos) -> (Sudoku, Pos)
solveHelp (sud, (8, 8)) = 
    (update sud (8, 8) (selectCell' (8, 8) sud cells), (8, 8))
    where cells = possibleCells (8, 8) sud
--solveHelp (sud, pos) | selectCell' pos sud cells == Nothing = (sud, pos)
solveHelp (sud, pos) | currCell == Nothing = solveHelp (sudup, next)
    | otherwise = solveHelp (sud, next)
    where cells = possibleCells pos sud
          sudup = (update sud pos (selectCell' pos sud cells))
          next = nextPos pos
          currCell = getCell sud pos
          impossible = selectCell' pos sud cells == Nothing
    
getCell :: Sudoku -> Pos -> Cell
getCell sud (y, x) = (extractRow sud y) !! x

solveHelp2 :: Sudoku -> Int -> Sudoku
solveHelp2 sud 10 = solved
    where solved = (fst (solveHelp (sud, (0, 0))))
solveHelp2 sud n | nblanks > 0 = solveHelp2 nextsud (n + 1)
                 | otherwise = solved
    where solved = (fst (solveHelp (sud, (0, 0))))
          nblanks = length (blanks solved) 
          nextsud = (update sud (0,0) (Just n))

solve :: Sudoku -> Sudoku
solve sud = solveHelp2 sud 6
